...
const inspectRuntimePrivileges = `-- name: InspectRuntimePrivileges :one
WITH required AS (
...
type InspectRuntimePrivilegesRow struct {
	SchemaPresent bool
	TablesPresent bool
	Permissions   bool
}
...
func (q *Queries) InspectRuntimePrivileges(ctx context.Context) (InspectRuntimePrivilegesRow, error) {
	row := q.db.QueryRow(ctx, inspectRuntimePrivileges)
	var i InspectRuntimePrivilegesRow
	err := row.Scan(&i.SchemaPresent, &i.TablesPresent, &i.Permissions)
	return i, err
}
